...
foreach v $allvtfs {
	...
	if {[string match "00-*" $molname] || [string match "01-*" $molname] || [string match "08-*" $molname] || [string match "10-*" $molname] || [string match "11-*" $molname] || [string match "12-*" $molname]|| [string match "15-*" $molname]}|| [string match "16-*" $molname]}|| [string match "17-*" $molname]}|| [string match "18-*" $molname]}|| [string match "22-*" $molname]}|| [string match "22-*" $molname]}|| [string match "24-*" $molname]}|| [string match "25-*" $molname]}|| [string match "26-*" $molname]}|| [string match "27-*" $molname]}|| [string match "28-*" $molname]}|| [string match "29-*" $molname]}|| [string match "30-*" $molname]}|| [string match "31-*" $molname]}|| [string match "32-*" $molname]}|| [string match "33-*" $molname]}} {
		source ../../Scripts/setOC1ins.tcl
		set $substrate "Insulin"
	} elseif {[string match "02-*" $molname] || [string match "05-*" $molname]} {
		source ../../Scripts/setOO1ins.tcl
		set $substrate "Insulin"
	} elseif {[string match "04-*" $molname]} {
		source ../../Scripts/setOO0ins.tcl
		set $substrate "None"
	} elseif {[string match "06-*" $molname]} {
		source ../../Scripts/setOpO0ins.tcl
		set $substrate "None"
	} elseif {[string match "07-*" $molname] || [string match "09-*" $molname]} {
		source ../../Scripts/setOpO1ins.tcl
		set $substrate "Insulin"
	} elseif {[string match "13-*" $molname] || [string match "14-*" $molname]} {
		source ../../Scripts/setOC0abeta.tcl
		set $substrate "ABeta"
	} else {
	...
	set coordout [open "$molname_coords.txt" w+]
	puts $coordout "Mol	Frame	Domain	X	Y	Z"
